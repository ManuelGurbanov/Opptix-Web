...
export default function ContractButton({language}) {
  const [contact, setContact] = useState(false);
    return (
        <>
        <button className=' bg-black text-nowrap rounded-3xl px-4 py-2 text-white flex items-center justify-center sm:gap-4 hover:scale-105 transition duration-75 sm:text-base text-xs' onClick={() => setContact(true)}>
