...
function RackModel({ showingMeshes, onMeshesLoaded, selectedTextures }) {
  ...
  const originalMaterials = useRef(new Map()); // Usamos useRef en lugar de useState para evitar re-renders
  const [groupTextures, setGroupTextures] = useState({}); // Almacena las texturas aplicadas
      ...
      >
        ...
        <directionalLight position={[0, -10, 0]} intensity={0.3} />
                ...
                <section className="absolute sm:flex flex-col hidden items-center justify-center top-4 right-7 gap-2">
                  <header className="text-lightblue font-bold text-2xl flex items-center justify-center gap-2">
